...
function TrendingBio(props) {

    const [bio, setBio] = useState([]);

    useEffect(() => {
        axios.get('http://localhost:4567/api/getAllTrending')
            .then((res) => {
                    ...
                    bio.map((item) => (
                        <div className="my-[80px] ml-3 mr-3  md:flex  items-center  h-auto bg-white bg-opacity-75  rounded-xl">
                            <div key={item._id} className="text-white  h-[250px] md:h-[200px] lg:w-[30%] md:w-[40%] bg-red-400 w-full rounded-xl ">
                                <img src={item.img} alt="Image" className='h-full w-full rounded-xl' />
                            </div>
                            <div className="mx-auto">
                                ...
                                <div className="text-white text-center mt-3">
                                    <Link to={item.path}><button className='bg-cyan-600 md:text-white   px-4 py-3 bg-opacity-95 font-extrabold rounded-lg w-full'> See More Info</button></Link>
                                </div>
                            </div>
                        </div>
                    ))
...
export default Layout()(TrendingBio);