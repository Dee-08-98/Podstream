...
function BuisnessCategoryID() {
    const { id } = useParams();
    const [bio, setBio] = useState(null);

    useEffect(() => {
        axios.post(`http://localhost:4567/api/bioCategory/buisnessCategory/${id}`)
            .then((res) => {
            ...
            {bio ? (
                <>
                    <div className="w-full lg:px-28 py-8">
                        <div className=" px-2 sm:flex  mb-8 w-full">

                            {bio.imageHeader.map((item, index) => (
                                <div key={index} className=" h-80 px-1 mt-2 w-full md:w-[50%]">
                        ...
                        <div className="hidden md:block">
                            <div className="flex flex-wrap gap-2">
                                ...
                                {bio.imageGallery.map((item, index) => (
                                    <div key={index} className=" mx-1 sm:w-1/2 md:w-1/3 p-2 h-40 w-full">
...
export default BuisnessCategoryID;
