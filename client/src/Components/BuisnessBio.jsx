...
function BuisnessBio() {
    const [bio, setBio] = useState([]);

    useEffect(() => {
        axios.get('http://localhost:4567/api/bioCategory/buisnessData')
            .then((res) => {
                        ...
                        <div className="my-[80px] ml-3 mr-3  md:flex  items-center  h-auto bg-white bg-opacity-75  rounded-xl">
                            ...
                            <div className="mx-auto">
                                ...
                                <div className="text-white text-center mt-3">
                                    <Link to={`/buisnessCategory/${item._id}`}><button className='bg-cyan-600 md:text-white   px-4 py-3 bg-opacity-95 font-extrabold rounded-lg w-full'> See More Info</button></Link>
                                </div>
                            </div>
                        </div>
                    ))
